...
function EditPasswordComponent() {
    ...
    const [confirmNewPassword, setConfirmNewPassword] = useState(''); // 새로운 비밀번호 재확인
    const [step, setStep] = useState(1); // 과정 상태
    ...
    const verifyCurrentPassword = async () => {
        try {
            const response = await userService.confirmPassword(userId, currentPassword, token); // 패스워드 확인
            const { data } = response;
    ...
    const updatePassword = async () => {
        if (newPassword !== confirmNewPassword) { // 둘의 패스워드가 불일치
            alert('패스워드가 일치하지 않습니다.');
            return;
        }
        try {
            const res = await userService.updateUserPassword(userId, newPassword, token);//패스워드 업데이트

            if(res.data.result === 'a breach of rules') // KISA 가이드라인을 준수했을시..
            {
